...
namespace GildedFate.Editor
{
    ...
    public sealed class ReaperEditorVideoSurface : IDisposable
    {
        ...
        public void Play(string name,float seconds,bool repeat)
        {
            ...
            clip=ReaperVideoCatalog.Find(name);loop=repeat;
            try
            {
                if(clip==null||!File.Exists(clip.Path))throw new IOException("Missing Reaper clip: "+name);
                ...
                if(!shader||!shader.isSupported)throw new NotSupportedException("Reaper alpha shader is unavailable.");
        ...
        {
            if(desired!=frame+1)
                if(!(bool)seek.Invoke(decoder,new object[]{new MediaTime(desired,24)}))throw new IOException("Cannot seek Reaper preview: "+clip.Name);
            var args=new object[]{packed,null};
            if(!(bool)nextFrame.Invoke(decoder,args))throw new IOException("Cannot decode Reaper preview: "+clip.Name);
