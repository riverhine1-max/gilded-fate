...
namespace GildedFate.Editor
{
    ...
    public sealed class VanguardEditorVideoSurface : IDisposable
    {
        ...
        RenderTexture keyed;
        Material material;
        VanguardVideoCatalog.Clip clip;
        ...
        public void Play(string name,float seconds,bool repeat)
        {
            ...
            clip=VanguardVideoCatalog.Find(name);loop=repeat;
            try
            {
                if(clip==null||!File.Exists(clip.Path))throw new IOException("Missing Vanguard clip: "+name);
                ...
                if(!shader||!shader.isSupported)throw new NotSupportedException("Vanguard alpha shader is unavailable.");
        ...
        {
            if(desired!=frame+1)
                if(!(bool)seek.Invoke(decoder,new object[]{new MediaTime(desired,24)}))throw new IOException("Cannot seek Vanguard preview: "+clip.Name);
            var args=new object[]{packed,null};
            if(!(bool)nextFrame.Invoke(decoder,args))throw new IOException("Cannot decode Vanguard preview: "+clip.Name);
