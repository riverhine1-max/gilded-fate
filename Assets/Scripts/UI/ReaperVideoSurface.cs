...
namespace GildedFate.UI
{
    ...
    public sealed class ReaperVideoSurface : IDisposable
    {
        private sealed class Channel
        {
            public VideoPlayer Player;
            public ReaperVideoCatalog.Clip Clip;
        ...
        public ReaperVideoSurface()
        {
            host=new GameObject("Reaper video presentation"){hideFlags=HideFlags.HideAndDontSave};
            var shader=Resources.Load<Shader>("ReaperPackedAlpha");
            if(shader&&shader.isSupported)keyMaterial=new Material(shader){hideFlags=HideFlags.HideAndDontSave};
            else lastError="Reaper alpha shader unavailable; keeping illustrated fallback.";
        ...
        private Channel CreateChannel(string label)
        {
            ...
            p.errorReceived+=(source,message)=>{ch.Failed=true;ch.FrameReady=false;lastError=label+": "+message;Debug.LogWarning("[Reaper Video] "+lastError);};
            return ch;
        }
        private void Load(Channel ch,string name,float speed,bool loop)
        {
            if(disposed)return;
            var clip=ReaperVideoCatalog.Find(name);
            ...
            if(clip==null||!File.Exists(clip.Path)){ch.Failed=true;lastError="Missing Reaper clip: "+name;return;}
        ...
        public void Play(string name,float seconds=.9f,bool loop=false)
        {
            ...
            Load(action,name,(ReaperVideoCatalog.Find(name)?.Duration??3f)/Mathf.Max(.4f,seconds),loop);
        }
        ...
        private void Composite(Channel ch)
        {
            ...
            {
                Release(ch);
                ch.Keyed=new RenderTexture(texture.width/2,texture.height,0,RenderTextureFormat.ARGB32){name="Reaper keyed "+ch.Clip.Name,hideFlags=HideFlags.HideAndDontSave};
