...
namespace GildedFate.UI
{
    ...
    public sealed class VanguardVideoSurface : IDisposable
    {
        private sealed class Channel
        {
            public VideoPlayer Player;
            public VanguardVideoCatalog.Clip Clip;
        ...
        public VanguardVideoSurface()
        {
            host=new GameObject("Vanguard video presentation"){hideFlags=HideFlags.HideAndDontSave};
            var shader=Resources.Load<Shader>("VanguardPackedAlpha");
            if(shader&&shader.isSupported)keyMaterial=new Material(shader){hideFlags=HideFlags.HideAndDontSave};
            else lastError="Vanguard alpha shader unavailable; keeping illustrated fallback.";
        ...
        private Channel CreateChannel(string label)
        {
            ...
            p.errorReceived+=(source,message)=>{ch.Failed=true;ch.FrameReady=false;lastError=label+": "+message;Debug.LogWarning("[Vanguard Video] "+lastError);};
            return ch;
        }
        private void Load(Channel ch,string name,float speed,bool loop)
        {
            if(disposed)return;
            var clip=VanguardVideoCatalog.Find(name);
            ...
            if(clip==null||!File.Exists(clip.Path)){ch.Failed=true;lastError="Missing Vanguard clip: "+name;return;}
        ...
        public void Play(string name,float seconds=.9f,bool loop=false)
        {
            ...
            Load(action,name,(VanguardVideoCatalog.Find(name)?.Duration??3f)/Mathf.Max(.4f,seconds),loop);
        }
        ...
        public void Tick(bool pause,bool lowHealth=false)
        {
            if(disposed)return;
            paused=pause;
            var idleName=lowHealth?"LowHPIdle":"CombatIdle";
        ...
        private void Composite(Channel ch)
        {
            ...
            {
                Release(ch);
                ch.Keyed=new RenderTexture(texture.width/2,texture.height,0,RenderTextureFormat.ARGB32){name="Vanguard keyed "+ch.Clip.Name,hideFlags=HideFlags.HideAndDontSave};
